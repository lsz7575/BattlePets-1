...
import com.cosmosnode.battlepets.versions.v1_8_1.Spawning_v1_8_R1;
...
public class BattlePets extends JavaPlugin implements Listener {
    ...
    public void loadConfigs() {
        ...
        final YamlConfiguration petsConfig = YamlConfiguration.loadConfiguration(petfile);
        conf.getKeys(false).forEach(string -> {
            if (!petsConfig.contains(string))
                petsConfig.set(string, conf.get(string));
        });
        try {
            petsConfig.save(new File(getDataFolder(), "pets.yml"));
        } catch (IOException e) {
            e.printStackTrace();
        }
